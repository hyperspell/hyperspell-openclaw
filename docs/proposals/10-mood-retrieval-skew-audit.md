...
> **Status (2026-07-12): audit tooling shipped on this branch.** The instrument follows the Loose Threads split (`lib/loops-audit.ts` + `scripts/audit-loops.ts`, PR #87) rather than the `docs/mood-skew-audit.mjs` shape sketched below: pure classification/bucketing/verdict logic lives in `lib/mood-skew-audit.ts` (hermetically tested in `npm test`), and the owner-run live runner is `scripts/audit-mood-skew.ts` (TypeScript via `node --experimental-strip-types`, never run by `npm test` or CI). Phase flags are `--mood` / `--census` / `--probe` as designed; the default run does all three plus the combined verdict against the pre-registered thresholds in §Test plan. Snapshot summaries print only under `HYPERSPELL_AUDIT_CONTENT=1` (the audit-loops / `HYPERSPELL_SCORE_LOG` privacy precedent) — run the `--mood` label review with it set. Two conscious narrowings from the sketch: the `memories.get` created_at sampling fallback (§Phase 2 step 3) is not implemented — the census reports `created_at` coverage explicitly and dates hot-buffer rows via the `openclaw_session_id` fallback, which covers the bulk of the corpus; and the per-resource character sizing (§Phase 2 step 4, already optional) is skipped. Production behavior is unchanged; the script is strictly read-only.
