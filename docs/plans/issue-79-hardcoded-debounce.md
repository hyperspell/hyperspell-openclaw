...
**⚠️ Verify against `origin/main` before implementing, not the local working tree.** `STORE_DEBOUNCE_MS` exists on `origin/main` (`hooks/emotional-state.ts:33`), but `TRACE_DEBOUNCE_MS` does **not** — it exists only in the maintainer's uncommitted local WIP to `hooks/auto-trace.ts` (which also adds the `lastTraceAt` map and session-cleanup handler this debounce depends on). On current `origin/main`, `auto-trace.ts` has no debounce at all and reads `event.sessionId` directly (the pre-issue-#42-fix duplicate-write bug). Confirm the WIP has landed — or land it yourself as a prerequisite — before implementing the `auto-trace.ts` half of this guide; the `emotional-state.ts` half is unaffected and can proceed independently.
...
**⚠️ Note re: Group B `proposal/14-enable-auto-trace` (external, informational only).** That proposal recommends flipping `autoTrace.enabled: true` on the live install and separately scopes "lift `TRACE_DEBOUNCE_MS` into config" as its own optional follow-up — i.e., the same change this guide makes. It shares this guide's WIP dependency (it presents the debounce/session-cleanup machinery as already shipped) and should be pointed at whichever of the WIP-landing or this guide lands first, rather than duplicating the config key under a different name.
