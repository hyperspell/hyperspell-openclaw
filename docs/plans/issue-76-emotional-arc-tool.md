...
Add a fourth agent-facing tool, `hyperspell_emotional_arc`, that lets the agent explicitly re-fetch the emotional register mid-conversation. Today the register only arrives via passive injection on the first session-start hook turn (`hooks/emotional-state.ts` → `buildEmotionalStateFetchHandler`); after compaction the injected block may be gone from history, and `injectedSessions` re-injection only helps on the *next* turn's prompt build — the agent has no way to pull the arc *now*, inside a turn.

**⚠️ Verify against `origin/main` before implementing, not the local working tree.** This guide (and #78) were drafted with `before_prompt_build` as the injection hook's name; `origin/main` still registers session-start injectors on `before_agent_start` (`index.ts`) — the rename exists only in the maintainer's uncommitted local WIP. The specific hook name doesn't change this guide's tool logic, but confirm the actual current registration point before writing the `index.ts` import/registration in Step 3, and don't assume other WIP-only surfaces (e.g. `hooks/auto-trace.ts`'s debounce machinery) have landed either.
...
**⚠️ Also note:** issue #72 (moodWeatherChance discoverability log) inserts a `log.info` call as content inside the same `if (cfg.emotionalContext) {` block in `index.ts` that Step 3 below adds `api.registerTool(...)` to. Trivial textual rebase only — whoever lands second adds their statement without deleting the other's. Separately, issue #78 (`/previewcontext`) exports the same two functions (`fetchRecentOrLatest`, `buildEmotionalContext`) from `hooks/emotional-state.ts` that Step 1 below exports — convergent, not conflicting: the second PR to land finds the exports already in place and skips that part of Step 1.
