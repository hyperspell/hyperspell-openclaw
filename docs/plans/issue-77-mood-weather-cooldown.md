...
**⚠️ Verify against `origin/main` before implementing.** `STORE_DEBOUNCE_MS`/`lastStoreAt` exist on `origin/main` (`hooks/emotional-state.ts:33`). `TRACE_DEBOUNCE_MS`/`lastTraceAt`/`buildAutoTraceSessionCleanupHandler`, cited above as the second existing precedent, exist only in the maintainer's uncommitted local WIP to `hooks/auto-trace.ts` — `origin/main`'s auto-trace has no debounce at all. This guide's own design (module-scope relationship-keyed map) doesn't depend on that precedent actually existing, but confirm the WIP has landed before citing it as shipped in the PR description, and don't assume auto-trace's other WIP-only behavior (the issue-#42 session-id fix) has landed either.
