...
| Emotional state (`hooks/emotional-state.ts`) | **No channel scoping at all.** Keyed by `relationshipId`; `deleteEmotionalState` is per-relationship, all-or-nothing. | Document as limitation. |
